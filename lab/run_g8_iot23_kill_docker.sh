...
MODEL_DIR=${1:?usage: bash lab/run_g8_iot23_kill_docker.sh MODEL_DIR PREPARED_DIR}
...
EVIDENCE=$(mktemp -d "$ROOT/artifacts/g8-iot23-kill.XXXXXXXX")
container=$(docker create --network none --cap-add NET_ADMIN --cap-add SYS_ADMIN --cap-add NET_RAW \
    ...
    omniguard-g8:local bash lab/container_g8_iot23_kill.sh)
cleanup() {
    docker cp "$container:/tmp/g8-iot23-kill/." "$EVIDENCE/" || true
    docker rm -f "$container" >/dev/null
    echo "Real-model IoT-23 kill evidence: $EVIDENCE"
}
trap cleanup EXIT
