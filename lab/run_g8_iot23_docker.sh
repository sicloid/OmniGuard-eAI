...
PREPARED_DIR=${2:?usage: bash lab/run_g8_iot23_docker.sh MODEL_DIR PREPARED_DIR [HOST_UDS_SOCKET]}
...
g10_mount=()
if [[ -n ${3:-} ]]; then
    G10_SOCKET=$(realpath -- "$3")
    [[ -S $G10_SOCKET ]] || { echo 'G10 socket must already be listening' >&2; exit 1; }
    g10_mount+=(--mount "type=bind,src=$(dirname -- "$G10_SOCKET"),dst=/run/g10-uds,readonly")
    g10_mount+=(--env "G10_EVENT_SOCKET=/run/g10-uds/$(basename -- "$G10_SOCKET")")
fi
