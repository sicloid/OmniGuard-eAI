...
| Which results can be claimed at delivery? | Per-window FPR budget, false quarantine and benign blocked time, leakage with censor reasons, data role and failed runs, exact run hashes. Linux x86_64 measurements are not Pi 5 results; Pi readings require same-boot load/temperature/throttling evidence. Distinguish V3 proposals from implemented behavior. | `docs/KAN42_MEASUREMENT.md`, `docs/REPRODUCE.md`, `docs/DEMO_RUNBOOK.md`, KAN-52/54/53 accepted records **when available**. |
